...
// The font lives in xoxo/fonts/ (source) → dist is 4 levels up from the
// compiled file, so we step back to the project root then into xoxo/fonts/.
try {
  const fontBuf = readFileSync(
    new URL('../../../../xoxo/fonts/JimNightshade-Regular.ttf', import.meta.url),
  );
