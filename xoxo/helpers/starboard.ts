...
async function countEligibleStars(reaction: any): Promise<number> {
  try {
...
export async function syncStarboardReaction(
  ...
  const emoji = currentReaction.emoji?.id
    ? `<${currentReaction.emoji.animated ? 'a' : ''}:${currentReaction.emoji.name}:${currentReaction.emoji.id}>`
    : (currentReaction.emoji?.name ?? 'unknown');
  console.log(`[starboard] Reaction received guild=${message.guild.id} channel=${channelId} message=${message.id} emoji=${emoji}`);
  if (message.author?.bot) {
    console.log(`[starboard] Ignored bot-authored message ${message.id}`);
    return;
  }
  ...
  if (!settings) {
    console.log(`[starboard] Ignored ${message.id}: no settings`);
    return;
  }
  if (settings.enabled === false) {
    console.log(`[starboard] Ignored ${message.id}: disabled`);
    return;
  }
  if (!settings.channel_id) {
    console.log(`[starboard] Ignored ${message.id}: no destination channel`);
    return;
  }
  if (!isConfiguredEmoji(currentReaction, settings)) {
    console.log(`[starboard] Ignored ${message.id}: emoji ${emoji} does not match configured ${settings.emoji}`);
    return;
  }
  if (ignored(message, settings)) {
    console.log(`[starboard] Ignored ${message.id}: source channel ${channelId} is on the ignore list`);
    return;
  }

  const count = await countEligibleStars(currentReaction);
  console.log(`[starboard] Count for ${message.id}: ${count}/${settings.threshold}`);
  ...
  if (!targetChannel?.isTextBased?.()) {
    console.log(`[starboard] Ignored ${message.id}: destination is not text-based`);
    return;
  }

  if (count < settings.threshold) {
    console.log(`[starboard] Waiting for more stars on ${message.id}: ${count}/${settings.threshold}`);
