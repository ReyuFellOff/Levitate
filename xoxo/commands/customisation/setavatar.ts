// xoxo/commands/customisation/setavatar.ts
//
// Change the bot's server avatar. Requires Administrator permission.
//
// Usage:
//   setavatar <image attachment OR image URL>
//   setavatar reset
...
async function patchAvatar(guildId: string, avatar: string | null, token: string): Promise<any> {
...
async function handle(
  ...
  const loadingMsg = await sendLoading(
    statusCtx,
    isReset ? 'Resetting server avatar...' : 'Setting server avatar...',
  );
...
export async function slashExecute(interaction: any, client: LevitateClient): Promise<any> {
  ...
  try {
    if (isReset) {
      await patchAvatar(interaction.guild.id, null, token);
      return sendSuccess({ interaction }, 'Server avatar reset to global avatar.');
    } else {
      const base64 = await imageUrlToBase64(attachment.url);
      await patchAvatar(interaction.guild.id, base64, token);
      return sendSuccess({ interaction }, 'Server avatar updated successfully.');
    }
  } catch (err: any) {
    console.error('[SETAVATAR]', err.message);
    return sendError({ interaction }, `Failed to update avatar: ${err.message}`);
  }
}
