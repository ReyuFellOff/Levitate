...
//
// Prefix:  $impersonate <@user|ID> <text>
//
// Checks:
...
// ─── Helper: resolve a user ID from a mention or raw snowflake ────────────────

function resolveId(token: string): string | null {
  const mention = token.match(/^<@!?(\d{17,20})>$/)?.[1];
  const bare    = /^\d{17,20}$/.test(token) ? token : null;
  return mention ?? bare;
}
...
export async function prefixExecute(
  ...
  // ── Argument validation ─────────────────────────────────────────────────────
  if (args.length < 2) {
    return sendError(ctx, 'Usage: `$impersonate <@user|ID> <text>`');
  }

  const userId = resolveId(args[0]);
  if (!userId) {
    return sendError(ctx, 'Please @mention a valid user or provide their user ID.');
  }
