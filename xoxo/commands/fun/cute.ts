// xoxo/commands/fun/cute.ts
//
// $howcute / $cute — rates how cute a user is.
//
// Usage:
//   $cute              — rates the author
//   $cute <@user|ID>   — rates the given user
//
// Special cases:
...
import { buildCutePayload } from '../../components/fun/cute.js';
import { resolveUser } from '../../helpers/userResolver.js';
import { resolveRatingPct } from '../../helpers/ratingBias.js';

export const options = {
  name:        'howcute',
  aliases:     ['cute'] as string[],
  description: 'See how cute someone is.',
  usage:       'cute\ncute <@user|ID|username>',
  category:    'fun',
  owner:       false,
  cooldown:    5,
};
...
export async function prefixExecute(
  ...
  const pct = await resolveRatingPct(client, 'cute', user.id, rollCuteness);

  const payload = await buildCutePayload({ user, pct, displayName });
...
export async function slashExecute(interaction: any, client: LevitateClient): Promise<any> {
  ...
  const pct = await resolveRatingPct(client, 'cute', rawUser.id, rollCuteness);
  const payload = await buildCutePayload({ user: rawUser, pct, displayName });
