// xoxo/commands/vcControls/deafen.ts
//
// Server-deafen a member in voice. Defaults to the invoker.
//
// Prefix:  $deafen [user]
// Slash:   /deafen [user]
...
export const options = {
  name:        'deafen',
  aliases:     [] as string[],
  description: 'Server-deafen a member in voice. Defaults to yourself.',
  usage:       'deafen [user]',
  category:    'vcControls',
  owner:       false,
  cooldown:    3,
...
async function handle(
  ...
  const deafened = await targetMember.voice.setDeaf(true).catch((): null => null);
  if (!deafened) return sendError(ctx, 'Failed to server-deafen that member.');
...
export async function prefixExecute(
  ...
  if (!botMember?.permissions?.has?.(PermissionFlagsBits.DeafenMembers))
    return sendError(ctx, 'I need the **Deafen Members** permission to server-deafen members.');
