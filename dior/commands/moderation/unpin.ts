...
export const options = {
  name:        'unpin',
  aliases:     [] as string[],
  description: 'Unpin a message by replying to it, message ID, or Discord link.',
...
export async function prefixExecute(
  ...
  if (!canManageMessages(target.channel, message.member)) {
    return sendError(ctx, 'You need the **Manage Messages** permission in the target channel to unpin messages.');
  }
  if (!canManageMessages(target.channel, botMember)) {
    return sendError(ctx, 'I need the **Manage Messages** permission in the target channel to unpin messages.');
  }
  if (!target.message.pinned) {
    return sendInfo(ctx, 'That message is not pinned.');
  }

  try {
    await target.message.unpin('Unpinned by moderation command.');
  } catch (error: unknown) {
    console.error(`[unpin] Failed to unpin message ${target.message.id}:`, error);
    return sendError(ctx, 'I could not unpin that message. Check my permissions and try again.');
  }

  return sendSuccess(ctx, `Unpinned [the message](${target.message.url}).`);
}