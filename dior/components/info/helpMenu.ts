...
/**
 * Build an update payload from the current help message with every
 * interactive component disabled. This is used when an interaction arrives
 * after the in-memory session has expired or been removed.
 */
export function buildDisabledHelpPayload(message: any) {
  const disableInteractiveComponents = (value: any): any => {
    if (!value || typeof value !== 'object') return value;

    const json = typeof value.toJSON === 'function' ? value.toJSON() : value;
    if (!json || typeof json !== 'object') return json;

    const copy: any = Array.isArray(json) ? [...json] : { ...json };
    if (Array.isArray(copy.components)) {
      copy.components = copy.components.map(disableInteractiveComponents);
    }

    // Buttons and all select-menu variants are interactive components.
    if ([2, 3, 5, 6, 7, 8].includes(copy.type)) {
      copy.disabled = true;
    }
  ...
  return {
    components: (message?.components ?? []).map(disableInteractiveComponents),
...
function buildHeaderSection(client: CassieClient, compact = false, guildId?: string | null): SectionBuilder {
  ...
  const descContent = compact
    ? descriptions.help.compact
    : descriptions.help.default;
...
function getDefaultAccentColor(): number {
  return parseInt(config.defaultAccentColor.replace('#', ''), 16);
}
...
const ALL_COMMANDS_TEXT_LIMIT = 3900;

function buildAllCommandsTextBlocks(
  ...
  for (const [cat, cmds] of sortedEntries) {
    ...
    // Keep the category divider visible without adding one component per
    // category (Discord caps the entire message at 40 components).
    const categoryBlock =
      `${emojis.blackButterfly} **${displayName}**\n` +
      `- ${cmdList}\n` +
      `-# ───────────────────────────────────────\n\n`;

    if (current && current.length + categoryBlock.length > ALL_COMMANDS_TEXT_LIMIT) {
  ...
  if (current) blocks.push(current.trimEnd());
  return blocks;
}
...
export async function buildAllCommandsPayload(
  ...
  const commandBlocks = buildAllCommandsTextBlocks(sortedEntries);
  const container = new ContainerBuilder()
    .setAccentColor(getDefaultAccentColor())
