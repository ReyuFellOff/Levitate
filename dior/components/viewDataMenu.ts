...
//
// Architecture mirrors helpMenu.ts / debug.ts:
//   • viewDataSessions — in-memory map keyed by the view-data message ID.
//   • buildViewDataPayload() — builds the CV2 listing with a StringSelectMenu
//     and Prev / Next pagination buttons (25 items per page).
//   • handleViewDataSelect() — user picks an item; fetches stored payload from
//     the storage channel, resolves placeholders, sends it as the right type.
//   • handleViewDataPage()   — user clicks Prev/Next; re-renders on new page.
//
// customId routing in interactionCreate.ts:
//   StringSelectMenu → 'viewdata:select'
//   Button           → 'viewdata:prev' | 'viewdata:next'
...
export interface ViewDataSession {
  userId:    string;
  guildId:   string;
  channelId: string;
  items:     SavedDataDoc[];
...
const viewDataTimeouts         = new Map<string, NodeJS.Timeout>();

export function registerViewDataSession(messageId: string, session: ViewDataSession): void {
  viewDataSessions.set(messageId, session);
  resetViewDataTimeout(messageId);
}

export function resetViewDataTimeout(messageId: string): void {
  const session = viewDataSessions.get(messageId);
  if (!session) return;

  clearTimeout(viewDataTimeouts.get(messageId));
  ...
  const timeout = setTimeout(async () => {
    try {
      ...
      await msg.edit(buildViewDataPayload(session.items, session.page, session.lastSent, true));
    } catch {
      // Message deleted or inaccessible — silently ignore
    } finally {
      viewDataSessions.delete(messageId);
      viewDataTimeouts.delete(messageId);
    }
  }, INACTIVITY_MS);

  viewDataTimeouts.set(messageId, timeout);
}
...
export function buildViewDataPayload(
  items:     SavedDataDoc[],
  page:      number,
  lastSent?: string,
  disabled = false,
  ...
  const footerLines: string[] = disabled
    ? ['-# This session has timed out. Run `$view-data` again.']
...
async function resolveViewDataSession(
  interaction: any,
): Promise<{ session: ViewDataSession; messageId: string } | null> {
  const messageId: string = interaction.message?.id ?? '';
  const session = viewDataSessions.get(messageId);

  if (!session) {
    await interaction.reply({
      content: 'This session has expired. Run `$view-data` again.',
...
export async function handleViewDataPage(
  ...
  await interaction.update(buildViewDataPayload(session.items, session.page, session.lastSent))
    .catch((): null => null);
  resetViewDataTimeout(messageId);
}
...
export async function handleViewDataSelect(
  ...
  session.lastSent = selectedName;
  await interaction.editReply(
    buildViewDataPayload(session.items, session.page, selectedName),
  ).catch((): null => null);
