...
| `$impersonate` | `imp`, `mimic` | Send a message as another server member via a temporary webhook (uses their server nickname and server avatar). Requires **Manage Messages** or **Administrator**. Bot needs **Manage Webhooks** in the channel. Webhook is deleted immediately after sending. Command message is deleted on success. Cooldown: 6 s. Category: `features`. |
